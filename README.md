...
> September 13: the encrypted delivery service is deployed and enabled; both physical phones still run build 9. Native Signal execution on iOS/Android and separate real TURN voice/video checks pass. The owner authorized AGPL-3.0-only source distribution. Binary distribution compliance, recovery and two-phone acceptance remain gates. [Current rollout and exact evidence](docs/DELIVERY_ROLLOUT.md).
...
Two physical iPhones: [current delivery status](docs/TWO_IPHONE_TESTING.md) and [Georgian test guide](docs/BETA_TEST_PLAN_KA.md). **0.1.0 (5)** passed signed archive/export/upload and Apple processing. It is **Waiting for Review** for the external Mnelo Preview group; automatic tester notifications are enabled, but no external email delivery or second-phone installation is claimed. The first owner already confirmed TestFlight build 4 opening and notification permission; build 5 is also assigned to the internal group for an in-place update. Two isolated, expiring Apple reviewer accounts are deployed and their protocol checks pass; normal SMS remains limited to the two real tester numbers. The original one identity and two Production push routes were preserved. Actual two-phone message/call/background receipt remains untested. Firebase Mnelo Development exists with Analytics disabled; Android client/sender configuration and independent production security review remain open.
...
UI typography and navigation: DM Sans/FiraGO are bundled locally, with a compact three-tab bar and complete active-tab highlight. See [font provenance](docs/TYPOGRAPHY.md) and the [September 12 QA result](docs/QA_REPORT.md). These source changes follow the already delivered TestFlight build 6 and need a subsequent mobile build to reach installed phones.
