...
> September 13: TestFlight **0.1.0 (12)** is uploaded and Apple processing is Complete. Mnelo Development (internal) explicitly shows **Testing**. Mnelo Preview (external) is **Waiting for Review**, with automatic notification enabled, replacing build 11 in the review queue. One phone is confirmed on build 11; update both phones to 12 in place before testing. Exact source is public under tag `ios-0.1.0-12`. [Build evidence](docs/BUILD_LOG.md) · [Physical corrections and test checklist](docs/PHYSICAL_FIXES.md).
