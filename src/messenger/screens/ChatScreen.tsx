...
import { FlatList, Linking, Keyboard, StyleSheet, View, type TextInput } from 'react-native';
...
import { richMedia, voteEmoji, type RichCard } from '../rich-message';
...
export function ChatScreen() {
  ...
  return (
    <MessageInfoNavigator key={`${identity?.key}:${id}`} chat={id}>
      <ChatConversationScreen />
    </MessageInfoNavigator>
  );
}
function ChatConversationScreen() {
  const messageInfo = useMessageInfoNavigation();
      ...
      >
        ...
        >
          ...
          <LocationProviderChoice
            ...
            onClose={() => setLocationProvider(false)}
            onChoose={(provider) => {
              if (!locationOrigin.current) return;
              if (provider === 'google') {
                void action.run(() =>
                  Linking.openURL('https://www.google.com/maps/@?api=1&map_action=map'),
                );
                return;
              }
