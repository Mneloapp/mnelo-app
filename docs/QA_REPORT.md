> **TestFlight September 13 — build 16:** signed archive/export/upload PASS at 20:20:03 Tbilisi. Version 0.1.0 (16) includes default Contacts-name permission flow, profile/number fallback and explicit saving to phone Contacts. All 484 tests and exact-source public CI pass. Apple reports the package uploaded and processing; the App Store Connect browser session expired before final processing, compliance, release notes and group availability could be verified. Owner reauthentication is pending. Do not claim build 16 is available to testers yet. Build 15 remains the last verified internal Testing / external Waiting for Review checkpoint. [Exact evidence](BUILD_LOG.md) · [Physical checks](PHYSICAL_FIXES.md).
