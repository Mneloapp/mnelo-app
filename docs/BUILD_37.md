...
The initial source tag `ios-0.1.0-37` passed the application checks but its CI dependency check detected a newly recommended `expo-build-properties` patch (57.0.19). The release uses `ios-0.1.0-37-r1`; the initial tag is retained unchanged and was not distributed as a binary. The patch adds opt-in scene support; inspection confirms its new iOS hook returns unchanged when the option is absent, as in Mnelo. Existing native settings and SDK remain unchanged.
