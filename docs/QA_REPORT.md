> **September 13 — TestFlight 0.1.0 (13):** signed archive/export/upload PASS. Public CI for exact source tag `ios-0.1.0-13` PASS; source checks total 455 tests. Final signatures, production APNs, disabled debugging, EN/KA purpose strings, defined QR provider, source/endpoint/UI markers and zero-finding native/Hermes secret scans verified. Mnelo Development is Testing; Mnelo Preview is Waiting for Review with automatic notification enabled. Build 12 was replaced in the beta-review queue. Native voice waveform/preview, contact navigation, menu motion, Photos/QR and upgraded two-phone delivery/calls remain physical tests. Four existing vendor dSYM warnings persist. [Full evidence](BUILD_LOG.md).
