> **TestFlight September 13 — build 15:** signed archive/export/upload PASS at 18:12:59 Tbilisi. Apple build `dd4aefee-4ba4-4704-94a9-6bab1677a1fa` is Complete; the existing standard-encryption / France No answers are saved. Mnelo Development (one internal tester) shows **Testing**; Mnelo Preview (one external tester) shows **Waiting for Review**, with automatic notification enabled. Build 15 replaced build 14 in the beta-review queue without expiring 14. Includes the phonebook-name access, number matching and live refresh fixes, alongside the previous profile/contact-library changes. All 472 tests and exact-source public CI pass. Update both phones in place to 15 without deleting Mnelo, then verify Contacts access/name selection. Build-15 installation and physical acceptance remain pending. [Exact build/source evidence](BUILD_LOG.md) · [Physical checklist](PHYSICAL_FIXES.md). This supersedes older distribution checkpoints below.
