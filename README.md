...
> September 13: TestFlight **0.1.0 (11)** is uploaded and Apple processing is Complete. It is assigned to Mnelo Development (internal) and submitted for Mnelo Preview (external), currently Waiting for Review, replacing the obsolete build 5 review. Both physical phones still run build 9; update both in place before testing. Exact source is public under tag `ios-0.1.0-11`. [Build evidence](docs/BUILD_LOG.md) · [Physical corrections and test checklist](docs/PHYSICAL_FIXES.md).
...
Two physical iPhones: [current delivery status](docs/TWO_IPHONE_TESTING.md) and [Georgian test guide](docs/BETA_TEST_PLAN_KA.md). **0.1.0 (11)** passes signed archive/export/upload and Apple processing. The existing internal Mnelo Development group has access; the external Mnelo Preview group is **Waiting for Review**, with automatic tester notification enabled. Build 5 was removed from review so the corrected version is reviewed. No external email delivery, build-11 installation or new two-phone flow PASS is claimed. Existing isolated reviewer accounts, real-number admission and hosted services were preserved. [Physical-test corrections](docs/PHYSICAL_FIXES.md) need acceptance on this new build.
