...
import {
  ...
  type PhonebookMatch,
} from '../../src/messenger/phonebook-match';
...
const session = new NotificationSession({
  ...
  request: fetch,
  async savedContacts(numbers, ownNumber) {
    ...
    for (const [number, raw] of rows) rememberPhonebookName(matches, number, raw);
    return matches;
  },
});
...
Object.assign(globalThis, {
  MneloShare: (id: string, operation: string, argument: string) => {
    const task =
      operation === 'preview'
        ? session.preview(argument, native<string>('language'))
        : session.close();
    void task
