> **TestFlight September 13 — build 14:** signed archive/export/upload PASS at 16:37 Tbilisi. Apple build `69db608f-68fd-4407-b614-b6a1136f420c` is Complete; compliance is cleared using the existing standard-encryption / France No answers. Mnelo Development (one internal tester) explicitly shows **Testing**; Mnelo Preview (one external tester) shows **Waiting for Review**, with automatic notification enabled. Build 14 replaced build 13 in the beta-review queue. Apple reports the internal tester Installed 0.1.0 (14); this is not two-phone functional acceptance. Includes the Me/Profile redesign and Contact info with separate Media/Links/Docs tabs, alongside the earlier physical-test corrections. All 466 automated tests and exact-source public CI pass. Both phones must update in place to 14 without deleting Mnelo. Build-14 physical acceptance is pending. [Exact build/source evidence](BUILD_LOG.md) · [Physical checklist](PHYSICAL_FIXES.md). This supersedes older distribution checkpoints below.
