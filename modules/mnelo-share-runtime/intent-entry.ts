...
const session = new IntentMessageSession({
  db: {
    async exec(sql) {
      native('exec', { sql });
    },
...
Object.assign(globalThis, {
  MneloShare: (id: string, operation: string, argument: string) => {
    const task = (async () => {
      if (operation === 'close') return session.close();
      const input: unknown = JSON.parse(argument);
      if (operation === 'intentResolve') return session.resolve(input);
      if (operation === 'intentConfirm') return session.confirm(input);
      if (operation === 'intentSend') return session.send(input);
      throw new Error('INTENT_OPERATION_INVALID');
    })();
    void task
